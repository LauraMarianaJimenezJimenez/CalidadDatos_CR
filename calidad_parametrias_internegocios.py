...
def MIS_PAR_INTER_SEG_ENG():

	try:
		# Create path
		file = os.path.abspath('../Fuentes_iniciales/MIS_PAR_INTER_SEG_ENG_' + data_date + '.xlsx')
		# Raed file
		df = pd.read_excel(file, header=None)

		print('Procesando MIS_PAR_INTER_SEG_ENG...')
		...
		try:
			...
			try:
				path = os.path.abspath('../Informes/Informe_MIS_PAR_INTER_SEG_ENG.txt')
				...
				file = os.path.abspath('../Fuentes_procesadas/MIS_PAR_INTER_SEG_ENG_' + data_date + '_' + unix_time + '.xlsx')
				writer = ExcelWriter(file)
				df.to_excel(writer, 'Hoja 1', index=False)
				writer.save()

				print("Paramteria MIS_PAR_INTER_SEG_ENG procesada con exito")
			...
			print(' Ha ocurrido un error, por favor verifique que los titulos de la parametria MIS_PAR_INTER_SEG_ENG sean [cod_entity, cod_gl_group, cod_acco_cent, cod_currency, cod_driver]')
			print(e)			
...
def MIS_PAR_INTER_SEG_DRI():

	try:
		
		print('Procesando MIS_PAR_INTER_SEG_DRI...')

		# Create path
		file = os.path.abspath('../Fuentes_iniciales/MIS_PAR_INTER_SEG_DRI_' + data_date + '.xlsx')
		# Raed file
		df = pd.read_excel(file, header=None)
		...
		try:
			...
			df = df.rename(columns=lambda x: x.strip())
			# Take specific columns
			df = df[['cod_driver','cod_segment','cod_blce_prod','allocation_perc']]
			x = len(df)
			# Remove duplicate records
			df = df.drop_duplicates()
			y = len(df)
			...
			try:
				path = os.path.abspath('../Informes/Informe_MIS_PAR_INTER_SEG_DRI.txt')
				...
				# Se escribe un nuevo archivo con la fuente procesada 
				file = os.path.abspath('../Fuentes_procesadas/MIS_PAR_INTER_SEG_DRI_' + data_date + '_' + unix_time + '.xlsx')
				writer = ExcelWriter(file)
				df.to_excel(writer, 'Hoja 1', index=False)
				writer.save()

				print("Paramteria MIS_PAR_INTER_SEG_DRI procesada con exito")
			...
			print(' Ha ocurrido un error, por favor verifique que los titulos de la parametria MIS_PAR_INTER_SEG_DRI sean [cod_driver, cod_acco_cent, cod_expense, %]')
			print(e)			
...
MIS_PAR_INTER_SEG_DRI()
MIS_PAR_INTER_SEG_ENG()