...
# Create path
file = 'C:\\Users\\laura.mariana.jimen1\\Documents\\Calidad_Datos_MIS_CR\\Fuentes_iniciales\\Curva_Captacion_Col_' + data_date +'.xls'
...
# Create output file
f = open("C:\\Users\\laura.mariana.jimen1\\Documents\\Calidad_Datos_MIS_CR\\Informes\\Informe_curva_captación_col_" + data_date + ".txt","w+")
f.write(file)
...
for column in df:
	...
	if i == 1:
		...
		df[column] = df[column].astype(str).str[:5]
		df[column] = df[column] + '%'
		df[column] = df[column].str.replace('.', ',', regex=False)
...
file = 'C:\\Users\\laura.mariana.jimen1\\Documents\\Calidad_Datos_MIS_CR\\Fuentes_procesadas\\Curva_Captacion_Col_' + data_date + '_' + unix_time + '.xls'
